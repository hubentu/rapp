...
#' @export
renderUI_bak <- function(ui, uid, Rfun = list(), dataList = list(), methodList = list(), outdir,
...
renderTemplate <- function(ui, app = TRUE, tmplJS){
    if(!is.list(ui)){
        ui <- list(ui)
    }
    if(app){
    ...
    tagList(vhtml, tags$script(paste0("\n", tmplJS, "\n")))
}
...
#' vue js
vueJS_bak <- function(ui, Rfun, outType = list("plot"), outID = list("plotOut"), dataList = list(), methodList = list()){
